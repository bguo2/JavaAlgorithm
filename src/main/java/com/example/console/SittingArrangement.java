...
The guests will sit down at a circular table which has n seats, numbered from 1 to n in clockwise order around the table.
 As the host, you will choose how to arrange the guests, one per seat. Note that there are n! possible permutations of seat
 assignments. Once the guests have sat down, the awkwardness between a pair of guests sitting in adjacent seats is defined as
 the absolute difference between their two heights. Note that, because the table is circular, seats 1 and n are considered to
 be adjacent to one another, and that there are therefore n pairs of adjacent guests.
...
public class SittingArrangement {
    ...
    public static int minOverallAwkwardness(int[] arr) {
        ...
        int max = 0;
        int pre = list.get(arr.length-1);
        for(int i = 0; i < arr.length; i++) {
            max = Math.max(max, Math.abs(list.get(i) - pre));
            pre = list.get(i);
        }
