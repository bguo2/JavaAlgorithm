...
// and "arts" are in the same group even though they are not similar.  Formally, each group is such that a word is in the group
...
public class SimilarStringGroup {
    ...
    public void dfs(String str, HashSet<String> visited, String[] strs)
    {
        if(visited.contains(str))
            return;
        visited.add(str);
        ...
        for(String s : strs)
        {
            if(!visited.contains(s) && isSimilar(s,str))
            {
                dfs(s, visited, strs);
            }
        }
