...
public class CountWords {
    public int wordCount(String[] startWords, String[] targetWords) {
        ...
        //use anagram can work: put startWords' anagram in the set. Then deleting 1 char one by one in the word of targetWords
        //the new string's anagram is in the set, then count it
        ...
        //we just need to delete 1 char in the target string, if it can be found in the set, then it is found
        for(String word: targetWords) {
            int bitmask = getBitmask(word);

            for(int i = 0; i < word.length(); i++) {
                int setBit = word.charAt(i) - 'a';
                //delete a char
                int newBitmask = bitmask & ~(1 << setBit);
